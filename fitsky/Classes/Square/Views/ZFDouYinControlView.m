...
- (void)showCoverViewWithUrl:(NSString *)coverUrl withImageMode:(UIViewContentMode)contentMode {
    ...
    [self.coverImageView setImageWithURLString:coverUrl placeholder:[UIImage imageNamed:@""]];// [UIImage imageNamed:@"icon_bg_ads_default"]
    [self.bgImgView setImageWithURLString:coverUrl placeholder:[UIImage imageNamed:@""]];
}
...
- (UIButton *)playBtn {
    if (!_playBtn) {
        ...
        [_playBtn setImage:[UIImage imageNamed:@"app-btn-play"] forState:UIControlStateNormal];
    }
    return _playBtn;
}
