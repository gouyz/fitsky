...
class FSModifyIntroductionVC: GYZWhiteNavBaseVC {

    /// 选择结果回调
    var resultBlock:((_ brief: String) -> Void)?
    
    //// 最大字数
    var contentMaxCount: Int = 20
    ///txtView 提示文字
    let placeHolder = "介绍一下自己吧！"
    var content: String = ""
    /// 是否是场馆修改名称
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = isVenue ? "场馆简介" : "个人简介"
    ...
    func requestModifyVueneProfileInfo(){
        ...
        GYZNetWork.requestNetwork("Admin/Store/edit", parameters: ["brief":content],  success: { (response) in
...
extension FSModifyIntroductionVC : UITextViewDelegate{
