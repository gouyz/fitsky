...
//    NSData *mvList = [NSKeyedArchiver archivedDataWithRootObject:model.mvList];
    ...
    [self.databaseQueue inDatabase:^(FMDatabase *db) {
        ...
        BOOL result = [db executeUpdate:@"INSERT INTO table_allresources (effectType,filterType, resourcePath, id, name, configFontId, configFontName, cnName, fontName, key, level, category, url, md5, banner, icon, description, isNew, preview, tag, cat, previewPic, previewMp4, duration, type, sort, pasterList) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", @(model.effectType),@(model.filterType), model.resourcePath, @(model.eid), model.name, @(model.configFontId), model.configFontName, model.cnName, model.fontName, model.key, @(model.level), model.category, model.url, model.md5, model.banner, model.icon, model.edescription, @(model.isNew), model.preview, model.tag, model.cat,model.previewPic, model.previewMp4, model.duration, model.type, model.sort, pasterList];
...
//    NSData *mvList = [NSKeyedArchiver archivedDataWithRootObject:model.mvList];
    ...
    [self.databaseQueue inDatabase:^(FMDatabase *db) {
        ...
        BOOL result = [db executeUpdate:@"UPDATE table_allresources SET resourcePath = ? AND pasterList = ? WHERE effectType = ? AND id = ? AND name = ?", model.resourcePath, pasterList, @(model.effectType), @(model.eid), model.name];
