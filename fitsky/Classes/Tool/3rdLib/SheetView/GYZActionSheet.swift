...
class GYZActionSheet: UIView {
    ...
    var itemBgColor : UIColor = UIColor.white{
        didSet{
            sheetView.cellBgColor = itemBgColor
        }
    }
    ///取消字体颜色,默认是blueColor
    ...
    fileprivate lazy var footerBtn : UIButton = {
        ...
        btn.backgroundColor = UIColor.UIColorFromRGB(valueRGB: 0x93b0f4)
    ...
    fileprivate func setupDefaultStyle(title: String){
        ...
        var titleCount = 0
        sheetView.divLine.isHidden = true
        if !title.isEmpty {
        ...
        if titleCount == 1 {
            ...
            sheetView.divLine.isHidden = false
        }
